...
class IterativeBacktest(IterativeBase):
    ...
    # helper method
    ## Fucntion for make long position
    def go_long(self, bar, units = None, amount = None):
       
        if self.position == -1:
            self.buy_instrument(bar, units = -self.units) # if short position, go neutral first
        
        if units: # option 1: we defined the units 
            self.buy_instrument(bar, units = units)
        
        elif amount: # option 2: we defined amount (that calculate how many units)
            if amount == "all":
    ...
    # helper method
    ## Fucntion for make short position
    ...
    ## Function for test (run) the cross SMA strategy
    ...
    ## Function for test (run) the momentume strategy   
    ...
    ## Function for testing the bolinger bounds strategy    
