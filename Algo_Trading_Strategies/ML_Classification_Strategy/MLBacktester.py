...
class MLBacktester():
    ...
    ## Function that return the string representation of the object
    def __repr__(self):
    ...
    def get_data(self):
        ...
        '''
        raw = pd.read_csv("../Data/five_minute_pairs.csv", parse_dates = ["time"], index_col = "time")
    ...
    def test_strategy(self, train_ratio = 0.7, lags = 5):
        ...
        full_data = self.data.copy() # get a copy of the data
        split_index = int(len(full_data) * train_ratio) # grab the splitting index 
        split_date = full_data.index[split_index-1] # extract the splitting date
